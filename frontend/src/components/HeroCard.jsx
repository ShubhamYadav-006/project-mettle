...
export default function HeroCard({
  ...
  if (loading) {
    return (
      <div className="mettle-panel rounded-md p-5 sm:p-7 bg-[var(--bg-surface)] border border-[var(--border)] animate-pulse space-y-6">
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          <div className="space-y-2.5">
            <div className="h-4 w-32 bg-[var(--bg-elevated)] rounded-sm" />
            <div className="h-7 w-56 sm:w-72 bg-[var(--bg-elevated)] rounded-sm" />
            <div className="h-4 w-64 sm:w-80 bg-[var(--bg-elevated)] rounded-sm" />
          </div>
          <div className="h-9 w-32 bg-[var(--bg-elevated)] rounded-sm shrink-0" />
        </div>

        <div className="space-y-2 pt-2 border-t border-[var(--border)]">
          <div className="flex justify-between items-center">
            <div className="h-4 w-24 bg-[var(--bg-elevated)] rounded-sm" />
            <div className="h-4 w-36 bg-[var(--bg-elevated)] rounded-sm" />
          </div>
          <div className="h-2 w-full bg-[var(--bg-elevated)] rounded-sm" />
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
          <div className="h-14 bg-[var(--bg-elevated)] rounded-sm" />
          <div className="h-14 bg-[var(--bg-elevated)] rounded-sm" />
          <div className="h-14 bg-[var(--bg-elevated)] rounded-sm" />
        </div>
      </div>
  ...
  if (error) {
    return (
      <div className="mettle-panel rounded-md p-6 bg-[var(--bg-surface)] border border-[var(--border)] text-center space-y-3">
        ...
        {onRetry && (
          <button
            onClick={onRetry}
            className="mt-2 inline-flex items-center gap-1.5 px-4 py-2 rounded-sm bg-[var(--bg-elevated)] hover:bg-[var(--bg-secondary)] border border-[var(--border-strong)] text-xs font-sans font-semibold text-[var(--text-primary)] hover:border-[var(--accent)] transition-all cursor-pointer"
          >
  ...
  return (
    <div className="mettle-panel rounded-md p-5 sm:p-7 bg-[var(--bg-surface)] border border-[var(--border)] shadow-sm space-y-6">
      {/* Eyebrow + Header + Message */}
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-5">
        <div className="space-y-1.5 max-w-2xl">
          {/* Eyebrow Badge */}
          <div className="inline-flex items-center gap-1.5 text-[10px] font-sans font-bold uppercase tracking-widest text-[var(--accent)] bg-[var(--accent-soft)] px-2.5 py-0.5 rounded-sm border border-[var(--accent-border)]">
        ...
        <div className="flex items-center gap-2 self-start md:self-auto shrink-0">
          <button
            ...
            className="px-4 py-2 rounded-sm bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-[var(--accent-text)] font-sans font-semibold text-xs uppercase tracking-wider active:scale-95 transition-all flex items-center gap-1.5 shadow-sm cursor-pointer"
          >
        ...
        <div className="flex flex-col sm:flex-row sm:items-baseline justify-between gap-1 text-xs">
          <div className="flex items-center gap-2">
            ...
            <span className="font-sans text-xs font-semibold text-[var(--text-secondary)]">
              {totalXp.toLocaleString()} XP <span className="text-[var(--text-muted)]">/ {nextLevelBaseXp.toLocaleString()} XP</span>
            </span>
          </div>

          <span className="text-[11px] font-medium text-[var(--text-muted)] font-mono">
            {xpRemaining > 0 ? `${xpRemaining.toLocaleString()} XP to Level ${level + 1}` : 'Max Tier'} ({progressPercent}%)
          </span>
        </div>

        {/* Animated XP Bar */}
        <div className="w-full h-2.5 rounded-sm bg-[var(--bg-elevated)] overflow-hidden border border-[var(--border)]">
          <div
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1">
        ...
        <div className="p-3.5 rounded-sm bg-[var(--bg-elevated)] border border-[var(--border)] flex flex-col justify-between">
          ...
          <div className="mt-1">
            {totalTodayCount > 0 ? (
              <span className="font-display text-base font-black text-[var(--text-primary)]">
                {completedTodayCount} of {totalTodayCount} quests completed
              </span>
            ) : (
        ...
        <div className="p-3.5 rounded-sm bg-[var(--bg-elevated)] border border-[var(--border)] flex flex-col justify-between">
          ...
          <div className="mt-1 flex items-center gap-1.5">
            <Flame className={`h-4 w-4 ${streak > 0 ? 'text-amber-500' : 'text-[var(--text-muted)]'}`} />
            {streak > 0 ? (
              <span className="font-display text-base font-black text-[var(--text-primary)]">
